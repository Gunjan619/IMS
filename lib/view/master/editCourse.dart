...
class _EditCourseState extends State<EditCourse> {
  ...
  EditCourseRequest editCourseRequest = EditCourseRequest("", "", "", "");
  ...
  List<String> items = ['Select Degree'];
    ...
    _service.degree(degreeRequest).then((value) {
      ...
      if (degreeResponse.error == false) {
        ...
        List.generate(degreeList.length, (index) {
          items.add(degreeList[index].name);
        });
        setState(() {
          isLoading = false;
        });
  ...
  void initializeDepartmentData(BuildContext context) {
    ...
    if (arguments['id'] != null) {
      ...
      degreeId = arguments['degreeId'];
      dropdownvalueService = arguments['degree'];

      setState(() {});
    } else {
      Navigator.pushReplacementNamed(context, '/ManageCourse');
    }
  }

  void _submitForm() {
    if (dropdownvalueService == 'Select Degree') {
      AppUtil.showToast("Please Select Degree", "e");
    } else if (nameController.text.isEmpty) {
      AppUtil.showToast("Please Enter Course Name", "e");
    } else if (yearController.text.isEmpty) {
      AppUtil.showToast("Please Enter No of Years", "e");
    } else {
      SharedPreferences.getInstance().then((SharedPreferences sp) {
        editCourseRequest.id = editId;
        editCourseRequest.degree = degreeId.trim();
        editCourseRequest.name = nameController.text.trim();
        editCourseRequest.year = yearController.text.trim();

        Webservice _service = Webservice();
        _service.editCourse(editCourseRequest).then((value) async {
