...
class ManageDegree extends StatefulWidget {
  const ManageDegree({super.key});

  @override
  State<ManageDegree> createState() => _ManageDegreeState();
}

class _ManageDegreeState extends State<ManageDegree> {
  int crocesscount = 0;
  ...
  void initState() {
    ...
    SharedPreferences.getInstance().then((SharedPreferences sp) {
      ...
      _service.degree(simpleRequest).then((value) {
  ...
  Widget build(BuildContext context) {
    ...
    } else if (permissions.contains('manage_degree')) {
      return SafeArea(
        child: Scaffold(
          appBar: AppBar(
            backgroundColor: Theme.of(context).primaryColor,
            actions: [
              if (permissions.contains('add_degree'))
                IconButton(
                  onPressed: (() {
                    Navigator.pushReplacementNamed(context, '/AddDegree');
                  }),
                  tooltip: "Add Degree",
                  icon: const Icon(
  ...
  callApiToDeleteOrganisation(String id) {
    ...
    _service.deleteDegree(deleteRequest).then((value) {
