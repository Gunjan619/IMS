...
class EditFeeName extends StatefulWidget {
  const EditFeeName({super.key});

  @override
  State<EditFeeName> createState() => _EditFeeNameState();
}

class _EditFeeNameState extends State<EditFeeName> {
  int crocesscount = 0;
  ...
  void initializeDepartmentData(BuildContext context) {
    ...
    if (arguments['id'] != null) {
      ...
      Navigator.pushReplacementNamed(context, '/ManageFeeName');
    }
  }

  void _submitForm() {
    if (nameController.text.isEmpty) {
      AppUtil.showToast("Please Enter Fee Name", "e");
    } else {
      SharedPreferences.getInstance().then((SharedPreferences sp) {
        ...
        service.editFeeName(editDegreeRequest).then((value) async {
  ...
  Widget build(BuildContext context) {
    ...
    } else if (permissions.contains('edit_fee_name')) {
      return SafeArea(
        child: Scaffold(
          key: _scaffoldKey,
          appBar: AppBar(
