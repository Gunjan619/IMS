...
class _ManageHolidayState extends State<ManageHoliday> {
  ...
  DeleteHolidayRequest deleteRequest = DeleteHolidayRequest("", "", "", "");
  ...
  String convertDate(String dateString) {
    ...
    DateTime parsedDate = DateFormat('d MMM, yyyy').parse(dateString);
  ...
  void initState() {
    ...
    SharedPreferences.getInstance().then((SharedPreferences sp) {
      ...
      _service.holiday(simpleRequest).then((value) {
        holidayResponse = value;

        if (value.error == false) {
          setState(() {
            holidayList = holidayResponse.data!;
            holidayFiltered = holidayList;
            isLoading = false;
          });
        } else {
          setState(() {
  ...
  Widget build(BuildContext context) {
    ...
    } else if (permissions.contains('manage_holiday')) {
      return SafeArea(
        child: Scaffold(
          appBar: AppBar(
            backgroundColor: Theme.of(context).primaryColor,
            actions: [
              if (permissions.contains('add_holiday'))
                IconButton(
                  onPressed: (() {
                    Navigator.pushReplacementNamed(context, '/AddHoliday');
                  }),
                  tooltip: "Add Holiday",
                  icon: const Icon(
                    ...
                    child: TextField(
                        ...
                        onChanged: (value) {
                          setState(() {
                            ...
                            holidayFiltered = holidayList
                                .where((holidayList) =>
                                    holidayList.holiday.toLowerCase().contains(
                                        _searchResult.toLowerCase()) ||
                                    holidayList.from.toLowerCase().contains(
                                        _searchResult.toLowerCase()) ||
                                    holidayList.to
                                        .toLowerCase()
                    ...
                    child: SizedBox(
                      child: Padding(
                        ...
                        child: isLoading
                            ...
                            : holidayFiltered.isNotEmpty
                                ? Container(
                                    padding: EdgeInsets.all(5.0),
                                    child: DynamicHeightGridView(
                                      itemCount: holidayFiltered.length,
                                      builder: (ctx, index) {
                                        return Card(
                                            color: Colors.white,
  ...
  void showMyDialog(String title, String content, String btnPositive,
      String btnNegative, bool cancelable, String id, String from, String to) {
    showGeneralDialog(
                          ...
                          Expanded(
                            ...
                            child: GestureDetector(
                              onTap: () {
                                ...
                                callApiToDeleteOrganisation(id, from, to);
                              },
                              child: Container(
  ...
  callApiToDeleteOrganisation(String id, String from, String to) {
    setState(() {
      isLoading = true;
    });
    deleteRequest.holiday = id;
    deleteRequest.type = 'delete';
    deleteRequest.from = from;
    deleteRequest.to = to;

    _service.deleteHoliday(deleteRequest).then((value) {
