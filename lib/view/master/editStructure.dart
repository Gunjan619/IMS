...
class _EditStructureState extends State<EditStructure> {
  ...
  final EditStructureRequest editStructureRequest =
      EditStructureRequest("", "", "", "", "", "");
  SimpleResponse editResponse = SimpleResponse();
  ...
  void initializeDepartmentData(BuildContext context) {
    ...
    if (arguments['id'] != null) {
      ...
      dropdownvalueService2 = arguments['courseName'];
      courseYear = arguments['courseYear'];
      yearId = arguments['subCourse'];
      dropdownvalueService3 = arguments['subCourseName'];
      sessionId = arguments['session'];
      dropdownvalueService = arguments['sessionName'];
      feesId = arguments['feeId'];
      dropdownvalueService4 = arguments['feeName'];
      nameController.text = arguments['amount'];
      ...
      _service.yearFilter(yearRequest).then((value) {
        ...
        if (yearResponse.error == false) {
          ...
          items3.add('Select Year');
          dropdownvalueService3 = items3[0]; // Reset dropdown
        }
      });

      sessionRequest.type = 'view';
      sessionRequest.year = courseYear;
      _service.sessionFilter(sessionRequest).then((value) {
        sessionResponse = value;
        if (sessionResponse.error == false) {
          sessionList = sessionResponse.data!;

          for (var sessions in sessionList) {
            items.add(sessions.session);
          }
          setState(() {
            isLoading = false;
          });
          ...
          setState(() {
            ...
            dropdownvalueService = items[0]; // Reset dropdown
            isLoading = false;
          });
        }
      });
    } else {
      Navigator.pushReplacementNamed(context, '/FeeStructure');
    }
  }

  void _submitForm() {
    if (dropdownvalueService2 == 'Select Course') {
      AppUtil.showToast("Please Select Course", "e");
    } else if (dropdownvalueService3 == 'Select Year') {
      AppUtil.showToast("Please Select Year", "e");
    } else if (dropdownvalueService == 'Select Session') {
      AppUtil.showToast("Please Select Session", "e");
    } else if (dropdownvalueService4 == 'Select Fee') {
      AppUtil.showToast("Please Select Fee", "e");
    } else if (nameController.text.isEmpty) {
      AppUtil.showToast("Please Enter Amount", "e");
    } else {
      editStructureRequest.id = editId.toString();
      editStructureRequest.course = courseId.toString();

      editStructureRequest.subCourse = yearId.toString();

      editStructureRequest.session = sessionId.toString();

      editStructureRequest.feeId = feesId.toString();

      editStructureRequest.amount = nameController.text.trim();

      Webservice _service = Webservice();
      _service.editStructure(editStructureRequest).then((value) async {
        editResponse = value;

        if (editResponse.error == false) {
          AppUtil.showToast(editResponse.msg!, "s");

          Navigator.pushReplacementNamed(context, '/FeeStructure');
        } else {
          AppUtil.showToast(editResponse.msg!, 'e');
        }
      });
      setState(() {});
    }
  ...
  Widget build(BuildContext context) {
    ...
    } else if (permissions.contains('edit_fee_structure')) {
      return isLoading
          ? SizedBox(
              width: double.infinity,
              height: double.infinity,
