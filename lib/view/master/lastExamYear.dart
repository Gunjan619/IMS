...
class LastExamYear extends StatefulWidget {
  const LastExamYear({super.key});

  @override
  State<LastExamYear> createState() => _LastExamYearState();
}

class _LastExamYearState extends State<LastExamYear> {
  int crocesscount = 0;
  ...
  LastExamYearResponse lastExamYearResponse = LastExamYearResponse();
  ...
  void initState() {
    ...
    SharedPreferences.getInstance().then((SharedPreferences sp) {
      ...
      _service.lastExam(simpleRequest).then((value) {
        lastExamYearResponse = value;

        if (value.error == false) {
          setState(() {
            yearList = lastExamYearResponse.data!;
            yearFiltered = yearList;
            isLoading = false;
          });
        } else {
          setState(() {
  ...
  Widget build(BuildContext context) {
    ...
    } else if (permissions.contains('manage_last_exam_year')) {
      return SafeArea(
        child: Scaffold(
          appBar: AppBar(
            backgroundColor: Theme.of(context).primaryColor,
            actions: [
              if (permissions.contains('add_last_exam_year'))
                IconButton(
                  onPressed: (() {
                    Navigator.pushReplacementNamed(context, '/AddLastExamYear');
                  }),
                  tooltip: "Add Year",
                  icon: const Icon(
                    ...
                    child: SizedBox(
                      child: Padding(
                        ...
                        child: isLoading
                            ...
                            : yearFiltered.isNotEmpty
                                ? Container(
                                    padding: EdgeInsets.all(5.0),
                                    child: DynamicHeightGridView(
                                      itemCount: yearFiltered.length,
                                      builder: (ctx, index) {
                                        return Card(
                                            color: Colors.white,
                                                                ...
                                                                child:
                                                                  ...
                                                                  onPressed:
                                                                    ...
                                                                    Navigator
                                                                      ...
                                                                      '/EditLastExamYear',
                                                                      arguments: {
                                                                        'id': yearFiltered[index]
                                                                            .id
                                                                            .toString(),
                                                                        'year': yearFiltered[index]
                                                                            .year
                                                                            .toString(),
                                                                      },
                                                                    );
  ...
  callApiToDeleteOrganisation(String id) {
    ...
    _service.deleteLastExam(deleteRequest).then((value) {
