...
class ManageSession extends StatefulWidget {
  const ManageSession({super.key});

  @override
  State<ManageSession> createState() => _ManageSessionState();
}
...
class _ManageSessionState extends State<ManageSession> {
  ...
  void initState() {
    ...
    SharedPreferences.getInstance().then((SharedPreferences sp) {
      ...
      _service.session(simpleRequest).then((value) {
        sessionResponse = value;

        if (value.error == false) {
          setState(() {
            sessionList = sessionResponse.data!;
            sessionFiltered = sessionList;
            isLoading = false;
          });
        } else {
          setState(() {
  ...
  Widget build(BuildContext context) {
    ...
    } else if (permissions.contains('manage_session')) {
      return SafeArea(
        child: Scaffold(
          appBar: AppBar(
            backgroundColor: Theme.of(context).primaryColor,
            actions: [
              if (permissions.contains('add_session'))
                IconButton(
                  onPressed: (() {
                    Navigator.pushReplacementNamed(context, '/AddSession');
                  }),
                  tooltip: "Add Session",
                  icon: const Icon(
                    ...
                    child: TextField(
                        ...
                        onChanged: (value) {
                          setState(() {
                            ...
                            sessionFiltered = sessionList
                                .where((sessionList) => sessionList.session
                                    .toLowerCase()
                    ...
                    child: SizedBox(
                      child: Padding(
                        ...
                        child: isLoading
                            ...
                            : sessionFiltered.isNotEmpty
                                ? Container(
                                    padding: EdgeInsets.all(5.0),
                                    child: DynamicHeightGridView(
                                      itemCount: sessionFiltered.length,
                                      builder: (ctx, index) {
                                        return Card(
                                            color: Colors.white,
                                                                ...
                                                                child:
                                                                  ...
                                                                  onPressed:
                                                                    ...
                                                                    Navigator
                                                                      ...
                                                                      '/EditSession',
                                                                      arguments: {
                                                                        'id': sessionFiltered[index]
                                                                            .id
                                                                            .toString(),
                                                                        'from': sessionFiltered[index]
                                                                            .from
                                                                            .toString(),
                                                                        'to': sessionFiltered[index]
                                                                            .to
                                                                            .toString(),
                                                                      },
                                                                    );
